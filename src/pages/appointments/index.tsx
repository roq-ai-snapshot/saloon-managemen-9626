...
export function AppointmentListPage(props: AppointmentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getAppointments({
        relations: ['user_appointment_guest_idTouser', 'organization', 'user_appointment_staff_idTouser'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['status.contains', 'service_type.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_guest_idTouser',
          header: 'User Appointment Guest Id Touser',
          accessorKey: 'user_appointment_guest_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_guest_idTouser?.id}`}
            >
              {record.user_appointment_guest_idTouser?.email}
            </Link>
          ),
        }
      : null,
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_staff_idTouser',
          header: 'User Appointment Staff Id Touser',
          accessorKey: 'user_appointment_staff_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_staff_idTouser?.id}`}
            >
              {record.user_appointment_staff_idTouser?.email}
            </Link>
          ),
        }
      : null,
